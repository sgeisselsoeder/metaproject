...
# own projects
for projectName in spielwiese 2025_ki3_nachklausur einsatzkraefte_drohnen_service nummernschild 2025_poster_projekte 2025_10_09_fit_tag generate_anomaly_data inference_benchmark 2026_03_19_ttz_fuck_up feuerdetektion kamerazentrale
do
	if [ ! -d "${projectName}" ]; then
...
mkdir -p ${GROUPNAME}${GITGROUPTAG}
# for projectName in chart-erkennung
for projectName in berichte gaia hautkrebs tetris-ai wearable_eeg InteractiveStoryTelling quantenki shyftai spiking_networks
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
		git clone git@${GITLAB_COM}:${SUB_URL}/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
	fi
...
SUB_URL="hsangit/research"
mkdir -p ${GROUPNAME}${GITGROUPTAG}
	for projectName in management perception_ai train_fdetect installer frontend mission_control
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
...
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
		echo 
		git clone git@github.com:aiengineeringblueprints/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
	fi
done
...
for projectName in fabric_spectroscopy_report
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
		# echo git@${GITLAB_COM}:hsangit/research/${GROUPNAME}/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
		git clone git@github.com:KIForAN/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
...
# git@github.com:KIForAN/fabric_spectroscopy_report.git
...
GROUPNAME="pawprint"
mkdir -p ${GROUPNAME}${GITGROUPTAG}
for projectName in 2024_studiprojekt pawai pawdata backend
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
		git clone git@${GITLAB_HS}:${GROUPNAME}/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
	fi
...
# for projectName in web_scraper
# do
# 	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
# 		git clone git@${GITLAB_HS}:tonihofmann/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
# 	fi
# done
for projectName in web_scraper
do
...
for projectName in dockerProjects multiscale spielDerNeuronen metaproject erkenntnis demo-self-driving webscrape templates-hs-ansbach  # km3netHdf5ToHistograms
do
	if [ ! -d "$projectName" ]; then
...
for projectName in drone_control_app neuraldrones nd_whip_server frat-automate dji_dock_frontend
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
...
for projectName in neuraldrones_infrared mavsdk_gazebo mavlink video_demo
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
...
projectName=quantum-tcn
if [ ! -d "${projectName}_own" ]; then
	git clone https://github.com/sgeisselsoeder/${projectName}.git ${projectName}_own
fi
