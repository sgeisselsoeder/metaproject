...
mkdir -p ${GROUPNAME}${GITGROUPTAG}
for projectName in hautkrebs drohne eegelektroden elektrodenmessung serviceroboter 3Danomaly shootair frontend bioreaktor genai_music tradebot
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
...
GROUPNAME="techdemos"
mkdir -p ${GROUPNAME}${GITGROUPTAG}
for projectName in pyscriptweb ansibleapp demo_sprachprotokoll retrain_yolo
do
	if [ ! -d "${GROUPNAME}${GITGROUPTAG}/${projectName}" ]; then
		git clone git@${GITLAB_HS}:${GROUPNAME}/${projectName}.git ${GROUPNAME}${GITGROUPTAG}/${projectName}
	fi
...
for projectName in 2022_sose_biolern_vorlesung 2023_lernverhalten 2023_04_27_forschungs_und_innovationstag bioreaktor \
	...
	reckenberg_wasser_solar data_connector_sftp
do
